...
# Batch use:
#
#     julia --project=. -t auto scripts/run_PTeosMHzGWs.jl 0 1 2
#
...
isdefined(@__MODULE__, :PTeosMHzGWs) || includet(joinpath(@__DIR__, "..", "src", "PTeosMHzGWs.jl"))
using .PTeosMHzGWs
...
gr()
# `pyplot()` also works, but PyPlot is not a dependency of this project — add it
# with `Pkg.add("PyPlot")` first. See `_savefig`, which repairs a log-axis bug in
# that backend. Set ENV["MPLBACKEND"] = "Agg" before including, or PyPlot tries
# to start a Qt backend and fails.

# ---------------------------------------------------------------------------
# Defaults — every one is overridable per call, none needs editing to switch file
# ---------------------------------------------------------------------------

# Input data. Defaults to the 120-sample extract in `data/sample/`, so a fresh
# clone runs with nothing else in place. Point `NS_FOPT_DATA` at a full
# EOSsampler run to reproduce the paper figures:
#
#     NS_FOPT_DATA=../EOSsampler/build/run500k julia --project=. -t auto
#
# Every function below also takes `datadir = ...` directly, so one call can read
# elsewhere without disturbing the default.
const DATA_DIR = get(ENV, "NS_FOPT_DATA", joinpath(@__DIR__, "..", "data", "sample"))
const OUT_DIR  = get(ENV, "NS_FOPT_OUT",  joinpath(@__DIR__, "..", "results"))
...
noise_curves() = (load_noise_curve(joinpath(@__DIR__, "..", "data", "noise_curves", "MWB-DMR-res.csv"), ','),
                  load_noise_curve(joinpath(@__DIR__, "..", "data", "noise_curves", "levitatedSensors_100m.csv"), ','),
                  load_noise_curve(joinpath(@__DIR__, "..", "data", "noise_curves", "LIGO5.tad"),columnf=1,columnasd=4),
                  load_noise_curve(joinpath(@__DIR__, "..", "data", "noise_curves", "ET-0001A-18_ETDSensitivityCurveTxtFile.txt")),
                  load_noise_curve(joinpath(@__DIR__, "..", "data", "noise_curves", "cosmic_explorer_strain.txt")))
