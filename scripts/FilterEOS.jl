...
function accepted_for(path; ids = nothing)
    ...
    h5open(path, "r") do f
        ...
        # `main` if this ever gets slow — one handle per file is the safe axis.
...
"""
    main(dir) -> Dict{String,Vector{Int}}

Scan every `*.h5` in `dir` and write the accepted ids to `accepted.json` beside
them, which is what the driver reads back through `accepted_ids`.

`dir` defaults to the sample dataset, so a fresh clone can run this end to end;
set `NS_FOPT_DATA` or pass the directory as the first argument to scan a full
EOSsampler run instead.
"""
function main(dir = get(ENV, "NS_FOPT_DATA", joinpath(@__DIR__, "..", "data", "sample")))
    files = sort(filter(endswith(".h5"), readdir(dir; join = true)))
    isempty(files) && error("no .h5 files found in $dir")
    ...
    out = joinpath(dir, "accepted.json")
    open(out, "w") do io
        JSON.print(io, results)
    end
    @info "wrote accepted.json" path = out total_accepted = sum(length, values(results))
    return results
end

# Guarded so that `include`ing this file in a session gives you `accepted_for`
# and `main` without scanning anything or overwriting an existing accepted.json.
if abspath(PROGRAM_FILE) == @__FILE__
    isempty(ARGS) ? main() : main(ARGS[1])
end
